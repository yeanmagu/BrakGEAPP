...
var e=b0.documentElement,f=b0.createElement("script"),j=b0.createElement("div"),d="script"+cn();
j.style.display="none";
j.innerHTML="   <link/><table></table><a href='/a' style='color:red;float:left;opacity:.55;'>a</a><input type='checkbox'/>";
var h=j.getElementsByTagName("*"),g=j.getElementsByTagName("a")[0];
if(!h||!h.length||!g){return
}ci.support={leadingWhitespace:j.firstChild.nodeType===3,tbody:!j.getElementsByTagName("tbody").length,htmlSerialize:!!j.getElementsByTagName("link").length,style:/red/.test(g.getAttribute("style")),hrefNormalized:g.getAttribute("href")==="/a",opacity:/^0.55$/.test(g.style.opacity),cssFloat:!!g.style.cssFloat,checkOn:j.getElementsByTagName("input")[0].value==="on",optSelected:b0.createElement("select").appendChild(b0.createElement("option")).selected,checkClone:false,scriptEval:false,noCloneEvent:true,boxModel:null};
f.type="text/javascript";
try{f.appendChild(b0.createTextNode("window."+d+"=1;"))
}catch(a){}e.insertBefore(f,e.firstChild);
if(a2[d]){ci.support.scriptEval=true;
delete a2[d]
}e.removeChild(f);
if(j.attachEvent&&j.fireEvent){j.attachEvent("onclick",function i(){ci.support.noCloneEvent=false;
...
var c=b0.createDocumentFragment();
c.appendChild(j.firstChild);
ci.support.checkClone=c.cloneNode(true).cloneNode(true).lastChild.checked;
ci(function(){var k=b0.createElement("div");
k.style.width=k.style.paddingLeft="1px";
b0.body.appendChild(k);
ci.boxModel=ci.support.boxModel=k.offsetWidth===2;
b0.body.removeChild(k).style.display="none";
k=null
});
var b=function(l){var k=b0.createElement("div");
l="on"+l;
var m=(l in k);
if(!m){k.setAttribute(l,"return;");
m=typeof k[l]==="function"
}k=null;
return m
};
ci.support.submitBubbles=b("submit");
ci.support.changeBubbles=b("change");
e=f=j=h=g=null
})();
...
}ci.each(["bind","one"],function(a,b){ci.fn[b]=function(f,g,c){if(typeof f==="object"){for(var e in f){this[b](e,g,f[e],c)
}return this
}if(ci.isFunction(g)){c=g;
g=bv
}var d=b==="one"?ci.proxy(c,function(h){ci(this).unbind(h,d);
return c.apply(this,arguments)
}):c;
return f==="unload"&&b!=="one"?this.one(f,g,c):this.each(function(){ci.event.add(this,f,d,g)
})
}
});
...
ci.each(["live","die"],function(a,b){ci.fn[b]=function(f,g,c){var e,d=0;
if(ci.isFunction(g)){c=g;
g=bv
}f=(f||"").split(/\s+/);
while((e=f[d++])!=null){e=e==="focus"?"focusin":e==="blur"?"focusout":e==="hover"?f.push("mouseleave")&&"mouseenter":e;
if(b==="live"){ci(this.context).bind(bu(e,this.selector),{data:g,selector:this.selector,live:e},c)
}else{ci(this.context).unbind(bu(e,this.selector),c?{guid:c.guid+this.selector+e}:null)
}}return this
}
});
...
}ci.each(("blur focus focusin focusout load resize scroll unload click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error").split(" "),function(a,b){ci.fn[b]=function(c){return c?this.bind(b,c):this.trigger(b)
};
if(ci.attrFn){ci.attrFn[b]=true
}});
...
}(function(){var s=b0.createElement("div"),t="script"+(new Date).getTime();
s.innerHTML="<a name='"+t+"'/>";
var u=b0.documentElement;
u.insertBefore(s,u.firstChild);
if(b0.getElementById(t)){h.find.ID=function(y,v,w){if(typeof v.getElementById!=="undefined"&&!w){var x=v.getElementById(y[1]);
return x?x.id===y[1]||typeof x.getAttributeNode!=="undefined"&&x.getAttributeNode("id").nodeValue===y[1]?[x]:bv:[]
}};
h.filter.ID=function(v,w){var x=typeof v.getAttributeNode!=="undefined"&&v.getAttributeNode("id");
return v.nodeType===1&&x&&x.nodeValue===w
}
}u.removeChild(s);
u=s=null
})();
(function(){var s=b0.createElement("div");
s.appendChild(b0.createComment(""));
if(s.getElementsByTagName("*").length>0){h.find.TAG=function(v,t){var w=t.getElementsByTagName(v[1]);
if(v[1]==="*"){var x=[];
for(var u=0;
w[u];
u++){if(w[u].nodeType===1){x.push(w[u])
}}w=x
}return w
}
}s.innerHTML="<a href='#'></a>";
if(s.firstChild&&typeof s.firstChild.getAttribute!=="undefined"&&s.firstChild.getAttribute("href")!=="#"){h.attrHandle.href=function(t){return t.getAttribute("href",2)
}
}s=null
})();
...
},parentsUntil:function(a,b,c){return ci.dir(a,"parentNode",c)
...
},nextUntil:function(a,b,c){return ci.dir(a,"nextSibling",c)
},prevUntil:function(a,b,c){return ci.dir(a,"previousSibling",c)
...
ci.each({appendTo:"append",prependTo:"prepend",insertBefore:"before",insertAfter:"after",replaceAll:"replaceWith"},function(a,b){ci.fn[a]=function(f){var e=[],c=ci(f);
for(var h=0,d=c.length;
h<d;
h++){var g=(h>0?this.clone(true):this).get();
ci.fn[b].apply(ci(c[h]),g);
e=e.concat(g)
...
ci.each("ajaxStart ajaxStop ajaxComplete ajaxError ajaxSuccess ajaxSend".split(" "),function(a,b){ci.fn[b]=function(c){return this.bind(b,c)
}
});
...
ci.each({slideDown:bM("show",1),slideUp:bM("hide",1),slideToggle:bM("toggle",1),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"}},function(a,b){ci.fn[a]=function(d,c){return this.animate(b,d,c)
}
});
...
}ci.each(["Height","Width"],function(a,b){var c=b.toLowerCase();
