...
var a0=l.documentElement,a1=l.createElement("script"),aV=l.createElement("div"),aZ="script"+Z();
aV.style.display="none";
aV.innerHTML="   <link/><table></table><a href='/a' style='color:red;float:left;opacity:.55;'>a</a><input type='checkbox'/>";
var a3=aV.getElementsByTagName("*"),a2=aV.getElementsByTagName("a")[0];
if(!a3||!a3.length||!a2){return
}p.support={leadingWhitespace:aV.firstChild.nodeType===3,tbody:!aV.getElementsByTagName("tbody").length,htmlSerialize:!!aV.getElementsByTagName("link").length,style:/red/.test(a2.getAttribute("style")),hrefNormalized:a2.getAttribute("href")==="/a",opacity:/^0.55$/.test(a2.style.opacity),cssFloat:!!a2.style.cssFloat,checkOn:aV.getElementsByTagName("input")[0].value==="on",optSelected:l.createElement("select").appendChild(l.createElement("option")).selected,checkClone:false,scriptEval:false,noCloneEvent:true,boxModel:null};
a1.type="text/javascript";
try{a1.appendChild(l.createTextNode("window."+aZ+"=1;"))
}catch(aW){}a0.insertBefore(a1,a0.firstChild);
if(K[aZ]){p.support.scriptEval=true;
delete K[aZ]
}a0.removeChild(a1);
if(aV.attachEvent&&aV.fireEvent){aV.attachEvent("onclick",function aU(){p.support.noCloneEvent=false;
aV.detachEvent("onclick",aU)
});
aV.cloneNode(true).fireEvent("onclick")
}aV=l.createElement("div");
aV.innerHTML="<input type='radio' name='radiotest' checked='checked'/>";
var aY=l.createDocumentFragment();
aY.appendChild(aV.firstChild);
p.support.checkClone=aY.cloneNode(true).cloneNode(true).lastChild.checked;
p(function(){var a4=l.createElement("div");
a4.style.width=a4.style.paddingLeft="1px";
l.body.appendChild(a4);
p.boxModel=p.support.boxModel=a4.offsetWidth===2;
l.body.removeChild(a4).style.display="none";
a4=null
});
var aX=function(a5){var a4=l.createElement("div");
a5="on"+a5;
var a6=(a5 in a4);
...
p.support.submitBubbles=aX("submit");
p.support.changeBubbles=aX("change");
a0=a1=aV=a3=a2=null
})();
...
}p.each(["bind","one"],function(aU,aV){p.fn[aV]=function(aX,aY,aZ){if(typeof aX==="object"){for(var aW in aX){this[aV](aW,aY,aX[aW],aZ)
}return this
}if(p.isFunction(aY)){aZ=aY;
aY=aM
}var a0=aV==="one"?p.proxy(aZ,function(a1){p(this).unbind(a1,a0);
return aZ.apply(this,arguments)
}):aZ;
return aX==="unload"&&aV!=="one"?this.one(aX,aY,aZ):this.each(function(){p.event.add(this,aX,a0,aY)
})
}
});
...
p.each(["live","die"],function(aU,aV){p.fn[aV]=function(aX,aY,aZ){var aW,a0=0;
if(p.isFunction(aY)){aZ=aY;
aY=aM
}aX=(aX||"").split(/\s+/);
while((aW=aX[a0++])!=null){aW=aW==="focus"?"focusin":aW==="blur"?"focusout":aW==="hover"?aX.push("mouseleave")&&"mouseenter":aW;
if(aV==="live"){p(this.context).bind(I(aW,this.selector),{data:aY,selector:this.selector,live:aW},aZ)
}else{p(this.context).unbind(I(aW,this.selector),aZ?{guid:aZ.guid+this.selector+aW}:null)
}}return this
}
});
...
}p.each(("blur focus focusin focusout load resize scroll unload click dblclick mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave change select submit keydown keypress keyup error").split(" "),function(aU,aV){p.fn[aV]=function(aW){return aW?this.bind(aV,aW):this.trigger(aV)
};
if(p.attrFn){p.attrFn[aV]=true
}});
...
if(l.getElementById(bd)){aV.find.ID=function(bi,bf,bg){if(typeof bf.getElementById!=="undefined"&&!bg){var bh=bf.getElementById(bi[1]);
return bh?bh.id===bi[1]||typeof bh.getAttributeNode!=="undefined"&&bh.getAttributeNode("id").nodeValue===bi[1]?[bh]:aM:[]
}};
aV.filter.ID=function(bf,bg){var bh=typeof bf.getAttributeNode!=="undefined"&&bf.getAttributeNode("id");
...
if(bc.getElementsByTagName("*").length>0){aV.find.TAG=function(bf,bd){var bg=bd.getElementsByTagName(bf[1]);
if(bf[1]==="*"){var bh=[];
for(var be=0;
bg[be];
...
if(bc.firstChild&&typeof bc.firstChild.getAttribute!=="undefined"&&bc.firstChild.getAttribute("href")!=="#"){aV.attrHandle.href=function(bd){return bd.getAttribute("href",2)
}
}bc=null
})();
...
},parentsUntil:function(aU,aV,aW){return p.dir(aU,"parentNode",aW)
...
},nextUntil:function(aU,aV,aW){return p.dir(aU,"nextSibling",aW)
},prevUntil:function(aU,aV,aW){return p.dir(aU,"previousSibling",aW)
...
p.each({appendTo:"append",prependTo:"prepend",insertBefore:"before",insertAfter:"after",replaceAll:"replaceWith"},function(aU,aV){p.fn[aU]=function(a1){var a0=[],aY=p(a1);
for(var aX=0,aZ=aY.length;
aX<aZ;
aX++){var aW=(aX>0?this.clone(true):this).get();
p.fn[aV].apply(p(aY[aX]),aW);
a0=a0.concat(aW)
...
p.each("ajaxStart ajaxStop ajaxComplete ajaxError ajaxSuccess ajaxSend".split(" "),function(aU,aV){p.fn[aV]=function(aW){return this.bind(aV,aW)
}
});
...
p.each({slideDown:y("show",1),slideUp:y("hide",1),slideToggle:y("toggle",1),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"}},function(aU,aV){p.fn[aU]=function(aX,aW){return this.animate(aV,aX,aW)
}
});
...
}p.each(["Height","Width"],function(aU,aV){var aW=aV.toLowerCase();
